...
- **2026-09-17 — Fix theme wipe (backdrop-filter desaparecía):** la View Transitions API crea un _stacking context_ en el snapshot del root, y `backdrop-filter` solo desenfoca contenido del mismo stacking context → el navbar y el ParallaxCard perdían su blur durante el wipe (bug de especificación del CSSWG, sin soporte browser aún). Fix: se reemplazó `document.startViewTransition` por un **iris wipe manual con WAAPI** — un overlay del color de fondo anterior (`--background`) se cierra en círculo revelando el nuevo tema, con el DOM **vivo** (los backdrop-filter nunca se pierden). Se quitó el CSS `::view-transition-*` de `globals.css`.
