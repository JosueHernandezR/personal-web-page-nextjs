...
const SuperformulaWireframe: React.FC = () => {
  ...
  const targetRotationRef = useRef<{ x: number; y: number; z: number }>({
    x: 0,
    y: 0,
    z: 0,
  });
  const currentRotationRef = useRef<{ x: number; y: number; z: number }>({
    x: 0,
    y: 0,
    z: 0,
  });
  ...
  const triggerBurst = (): void => {
    ...
    if (currentTime - state.lastBurstTime > 0.3) {
      ...
      if (
        ...
        wireframeMeshRef.current.material instanceof THREE.ShaderMaterial
      ) {
        const theme = state.themes[state.params.colorTheme];
        const burstColor = theme.burstColor || "#ffffff";

        wireframeMeshRef.current.material.uniforms.burstActive.value =
          state.burstActive;
        wireframeMeshRef.current.material.uniforms.burstStartTime.value =
          state.burstStartTime;
        wireframeMeshRef.current.material.uniforms.burstColor.value.set(
          burstColor,
        );
      }
    }
  ...
  const createWireframe = useCallback((): void => {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        microAnimationIntensity: {
          value: state.params.microAnimationIntensity,
        },
        dashSize: { value: 0.1 },
        dashRatio: { value: 0.5 },
        ...
        burstColor: {
          value: new THREE.Color(
            state.themes[state.params.colorTheme].burstColor,
          ),
        },
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- referencias adelantadas intencionales: startMorphing/updateWireframeGeometry se declaran después (agregarlas a deps causaría TDZ); el callback corre tras la inicialización
  }, []);
  ...
  const updateWireframeGeometry = useCallback((): void => {
    ...
    const sfParams = state.isMorphing
      ? getInterpolatedParams()
      : (state.currentPresetParams as PresetParams);
    const resTheta = state.resolutionTheta;
    const resPhi = state.resolutionPhi;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- getInterpolatedParams se declara después (agregarlo a deps causaría TDZ); el callback corre tras la inicialización
  }, []);
  ...
  const getInterpolatedParams = (): PresetParams => {
    if (!clockRef.current)
      return stateRef.current.currentPresetParams as PresetParams;
    ...
    if (
      ...
      wireframeMeshRef.current.material.uniforms.morphProgress.value =
        morphEffect;
    }
  ...
  const animate = useCallback((): void => {
    ...
    if (state.isMorphing) {
      const morphProgress =
        (elapsedTime - state.morphStartTime) /
        Math.max(0.001, state.params.morphDuration);
    ...
    if (wireframeMeshRef.current) {
      ...
      currentRotationRef.current.z +=
        (targetRotationRef.current.z - currentRotationRef.current.z) *
        rotationDamping;
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const distanceVector = new THREE.Vector3(0, 0, 0).sub(
  ...
  useEffect(() => {
    ...
    const themeInterval = setInterval(() => {
      const state = stateRef.current;
      const currentThemeIndex = state.themeNames.indexOf(
        state.params.colorTheme,
      );
      ...
      if (
        ...
        const themeColor =
          state.themes[state.params.colorTheme].burstColor || "#ffffff";
        wireframeMeshRef.current.material.uniforms.burstColor.value.set(
          themeColor,
        );
      }
