...
const CameraWireframe = () => {
  ...
  useEffect(() => {
    ...
    const createDetailedCameraMesh = () => {
      ...
      const createCameraBody = () => {
        ...
        const roundCorners = () => {
          ...
          for (let i = 0; i < positions.count; i++) {
            ...
            if (
              xAbs > halfWidth - cornerRadius &&
              yAbs > halfHeight - cornerRadius &&
              zAbs > halfDepth - cornerRadius
            ) {
        ...
        const createViewfinder = () => {
          ...
          viewfinderWireframe.position.set(
            0,
            bodyHeight / 2 + viewfinderHeight / 2,
            0
          );
        ...
        const createLCD = () => {
          ...
          for (let i = 0; i < lcdPositions.count; i++) {
            ...
            if (
              Math.abs(x) > halfWidth - cornerRadius &&
              Math.abs(y) > halfHeight - cornerRadius
            ) {
              const cornerX = Math.sign(x) * (halfWidth - cornerRadius);
              const cornerY = Math.sign(y) * (halfHeight - cornerRadius);
        ...
        const createDials = () => {
          ...
          dialDefs.forEach((dial) => {
            ...
            const detailGeometry = new THREE.CircleGeometry(
              dial.radius * 0.8,
              dial.segments
            );
        ...
        // CORREGIDO: Crear montura del lente con anillos centrados correctamente
        // pero sin cambiar su orientación original
        const createLensMount = () => {
          const mountRadius = 1.0;
          const mountDepth = 0.2;

          // Cilindro para la montura principal
          ...
          // Orientación con eje principal hacia Z
          mountWireframe.rotation.z = Math.PI / 2;
          mountWireframe.position.set(0, 0, -bodyDepth / 2 - mountDepth / 2);

          // Anillos internos de la montura también centrados pero con orientación original
          const innerRings = [
          ...
          innerRings.forEach((ring) => {
            ...
            // Manteniendo la orientación original pero centrando correctamente
            ringWireframe.position.set(0, 0, -mountDepth - ring.depth);
        ...
        const createHotShoe = () => {
          ...
          leftRail.position.set(
            -hsWidth / 2 + railWidth / 2,
            hsHeight / 2 + railHeight / 2,
            0
          );
        ...
        const createButtons = () => {
          ...
          sideButtonPositions.forEach((button) => {
            const buttonGeometry = new THREE.PlaneGeometry(
              button.width,
              button.height
            );
            const buttonWireframe = new THREE.LineSegments(
      ...
      const createLens = () => {
        ...
        // Crear secciones del lente con anillos correctamente orientados
        const createLensSections = () => {
          const sectionsGroup = new THREE.Group();

          // Definir secciones del lente
          const sections = [
          ...
          sections.forEach((section, index) => {
            ...
            // Orientación correcta con eje principal hacia Z
            sectionWireframe.rotation.x = Math.PI / 2;
            sectionWireframe.position.z = section.position;

            sectionsGroup.add(sectionWireframe);

            // Añadir anillos de detalle entre secciones con la orientación correcta
            if (index > 0) {
              const ringGeometry = new THREE.TorusGeometry(
                frontRadius + 0.02,
                0.03,
              ...
              // Mantener la orientación original de estos anillos
        ...
        // Create control rings (zoom, focus) with correct orientation
        ...
        // CORREGIDO: Reducir el espacio entre los anillos verdes y el cilindro del lente
        const createFrontElements = () => {
          ...
          glassWireframe.position.z = -4.35;

          frontGroup.add(glassWireframe);

          // CORREGIDO: Anillos verdes con menor espacio al cilindro del lente, pero misma orientación
          const greenRings = [
  ...
  { radius: maxRadius * 0.75, thickness: 0.03, position: -4.31 }
          ...
          greenRings.forEach((ring) => {
            ...
            // Manteniendo exactamente la misma rotación original
            ringWireframe.rotation.z = Math.PI / 2;
