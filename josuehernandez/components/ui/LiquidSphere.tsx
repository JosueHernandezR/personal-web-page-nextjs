  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (liquidMaterial && liquidMaterial.uniforms) {
        liquidMaterial.uniforms.mousePosition.value.set(mousePosition.current.x, mousePosition.current.y);
      }
      
      // Inclinación suave hacia la posición del ratón con mayor respuesta
      if (sphereRef.current) {
        const targetRotationX = mousePosition.current.y * 0.2 - 0.1;
        const targetRotationY = mousePosition.current.x * 0.2;
        
        sphereRef.current.userData = sphereRef.current.userData || {};
        sphereRef.current.userData.targetRotationX = targetRotationX;
        sphereRef.current.userData.targetRotationY = targetRotationY;
      }
    };
    ...
    const animate = () => {
      ...
      if (sphereRef.current && sphereRef.current.userData) {
        const targetX = sphereRef.current.userData.targetRotationX || -0.1;
        const targetY = sphereRef.current.userData.targetRotationY || 0;
